...
export default function Inventory(props) {
  ...
  const { domain, advanceSearchData, vehiclesData2, vehiclesData, dealerData } =
    props;
...
export async function getServerSideProps(ctx) {
  ...
  const { data: advanceSearchData, status: advanceSearchStatus } =
    await httpRequest(
      "GET",
      `/api/dealership/advance/search/vehicles/get/${domain}?is_it_sold=1`,
      {},
      {},
      false
    );
  ...
  if (
    ...
    const { status: vehiclesStatus2, data: vehiclesData2 } = await httpRequest(
      "GET",
      `/dealership/vehicles/all/with/thumbnail/${domain}?is_it_sold=1`,
      {},
      {},
      false
    );
