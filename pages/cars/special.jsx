...
export default function Inventory(props) {
  ...
  const { domain, advanceSearchData, vehiclesData, vehiclesData2, dealerData } =
    props;
  ...
  const body = {
    // url: domain,
    year_start: query?.Minyear ? query?.Minyear : "0", //query?.is_classic === "1" ? "0" : "1970",
    year_end: query?.Maxyear ? query?.Maxyear : currentYear, //query?.is_classic === "1" ? "1970" : currentYear,
    ...
    make: query?.make ? query?.make : "",
    model: query?.model ? query?.model : "",
    odometer_type: 2,
    transmission: "",
    body_style: query?.bodystyle ? query?.bodystyle : "",
    drive_train: "",
    doors: "",
    interior_color: "",
    Exterior_color: "",
...
export async function getServerSideProps(ctx) {
  ...
  const { data: advanceSearchData, status: advanceSearchStatus } =
    await httpRequest(
      "GET",
      `/api/dealership/advance/search/vehicles/get/${domain}?special=1`,
      {},
      {},
      false
    );
  ...
  if (
    ...
    const { status: vehiclesStatus, data: vehiclesData } = await httpRequest(
      "POST",
      `/api/dealership/advance/search/vehicles/${domain}?page=1&limit=10`,
      body,
      {},
      false
    );
    const { status: vehiclesStatus2, data: vehiclesData2 } = await httpRequest(
      "GET",
      `/dealership/vehicles/all/with/thumbnail/${domain}?is_it_special=1`,
      {},
      {},
      false
    );
