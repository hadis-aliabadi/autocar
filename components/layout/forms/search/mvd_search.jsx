...
const SearchForVehicle = (props) => {
  ...
  const handleSelectVehicle = (id, name) => {
    if (formik.values.frk_midv_id !== id) {
      formik.setFieldValue("desired_mid_vehicle", "");
      formik.setFieldValue("frk_midv_id", id);
    }
  ...
  const handleRemoveVehicle = () => {
    formik.setFieldValue("frk_midv_id", "");
    setSelectedVehicle();
  };
  return (
    <div className="p-0 m-0 w-100">
      <div
        className="form-group p-0 m-0 mt-2 mb-2"
      ...
      >
        ...
        />
        {formik.errors.frk_midv_id || formik.errors.desired_mid_vehicle ? (
          <>
            <small className="text-danger">{formik.errors.frk_midv_id}</small>
          </>
        ) : null}
        {formik.values.frk_midv_id && selectedVehicle !== null && (
                ...
                >
                  <div className="p-0 m-0 d-flex align-items-center">
                    <input
                      ...
                      id={vehicle?.name}
                      style={{ cursor: "pointer" }}
                      checked={formik.values.frk_midv_id === vehicle?.id}
                    />
